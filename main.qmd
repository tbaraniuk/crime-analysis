...
complaint_data2 <- read_parquet('data/NYPD_Complaint_Data_Historic_20251021.parquet')
...
evictions2 <- read_csv('data/Evictions_20251021.csv')
...
### how does crime complaints depend on evictions?
```{r}
#| code-fold: true
#| code-summary: "show code"
crime_by_year_borough <- complaint_data2 |>
  mutate(
    date = mdy(CMPLNT_FR_DT),
    year = year(date),
...
map_bi <- nyc_boroughs |> left_join(bi, by = "borough") |>
...
p_bi_static <- ggplot(map_bi) +
  ...
  geom_sf(aes(fill = bi_key, frame = year, ids = id, text = paste0("Район: ", borough,
                  "\nСкарги: ", complaints,
                  "\nВиселення: ", evictions2,
                  "\nРік: ", year)), color = NA, linewidth = 0.3) +
  scale_fill_manual(values = bi_pal, na.value = "grey80", name = "complaints × evictions") +
  labs(
    title = "how does crime complaints depend on evictions",
  ) +
  theme_void() +
  theme(
